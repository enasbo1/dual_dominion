    ...
    public class RandomSpeedManager : WalkerManager
    {
        ...
        private void Start()
        {
            for (int i = 0; i < Size; i++)
                _currentSpeed[i] = _animators[i].GetFloat(WalkerAnimP.WalkSpeed);
        }
        ...
        private void FixedUpdate()
        {
            for (int i = 0; i < Size; ++i)
                if (Active[i])
                {
                    ...
                    _animators[i].SetFloat(WalkerAnimP.WalkSpeed, _currentSpeed[i]);
                }
        }
        ...
        protected override void AddElementInChunk(WalkerDdDealer element)
        {
            ...
            _currentSpeed[Size] = element.animator?.GetFloat(WalkerAnimP.WalkSpeed) ?? 1f;
        }

        protected override void AddElementInNew(WalkerDdDealer element)
        {
            _animators.Add(element.animator);
            _currentSpeed.Add(element.animator?.GetFloat(WalkerAnimP.WalkSpeed) ?? 1f);
        }
    }
}