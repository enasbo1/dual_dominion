    ...
    public class FollowRotationManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            foreach (Transform follow in followers)
            {
                if (_hasRB[i])
                {
                    _rigidBodies[rb].rotation = RotationApply(follow, leaders[i].rotation.eulerAngles, _offsets[i],
                        speed[i], onlyY[i]);
                    ++rb;
                }
                else
                {
                    follow.rotation = RotationApply(follow, leaders[i].rotation.eulerAngles, _offsets[i], speed[i],
                        onlyY[i]);
                }
        ...
        private static void ApplyLimitedRotation(Transform targetTransform, Quaternion target, Quaternion start,
            float limit, bool onlyY)
        {
            if (limit == 0) return;
            Vector3 angle = (target * Quaternion.Inverse(start)).eulerAngles;
            float[] n = { angle.x, angle.y, angle.z };
            int i = 0;
        ...
        private static Quaternion RotationApply(Transform follower, Vector3 lead, Vector3 offset, float speed,
            bool onlyY)
        {
            Vector3 rot = follower.rotation.eulerAngles;
            Vector3 target = lead + offset;
            if (onlyY)
            {
                target.x = rot.x;
                target.z = rot.z;
