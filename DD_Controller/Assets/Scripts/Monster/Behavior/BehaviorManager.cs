...
namespace Monster.Behavior
{
    ...
    public class BehaviorManager : Manager<MonsterDealer, WalkerEnum, MonsterVariants>
    {
        ...
        {
            ActivesBehaviors.Add(MonsterBehaviorEnum.Start);
        ...
        {
            if (element.witnessBlessing != null) for (int j = 0; j < DefaultMaterials[i].Length; j++)
            {
        ...
        private void FixedUpdate()
        {
            ...
            for(int i = 0; i < Size; ++i) if (Active[i])
            {
                ...
                for (int j = 0; j < KnownBehaviors[i].Length; ++j) if (Available[j])
                {
                    if (rand == 0)
                    {
